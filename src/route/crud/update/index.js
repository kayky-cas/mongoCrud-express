...
route.post('/', (require, response) => {
    ...
    else if (typeof require.body.search === 'undefined') response.json({status: false, error: 108});
    else if (typeof require.body.change === 'undefined') response.json({status: false, error: 109});
                ...
                if (!result[0].permission) response.json({status: false, error: 103});
                    ...
                    AnimalController.select().then((resultA) => {
                        if (resultA.length === 0) response.json({status: false, error: 106});
                            ...
                            AnimalController.update(JSON.parse(require.body.search), JSON.parse(require.body.change)).then(() => {
